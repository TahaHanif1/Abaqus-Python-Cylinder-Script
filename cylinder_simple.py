...
def TopologyCylinder(fy):
    ...
    mdb.models['Model-1'].materials['Material-1'].Plastic(table=((fy, 0.0), ))
    ...
    print("Model with fy = %s is completed" % str(fy))


def PostProcessOptimization():
    mdb.CombineOptResults(optResultLocation=folder_path + "/" + str(fy) + "/" + 'Opt-Process-1', 
    ...
    o6 = session.openOdb(name=folder_path + "/" + str(fy) + "/" + 'Opt-Process-1/TOSCA_POST/Opt-Process-1-Job_post.odb')
...
for fy in [235.0, 275.0, 355.0, 430.0]:
    
    os.mkdir(folder_path + "/" + str(fy))
    os.chdir(folder_path + "/" + str(fy))
    TopologyCylinder(fy)
